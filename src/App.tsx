...
import { CelestialNavigationPanel } from '@/sections/CelestialNavigationPanel';
...
type Page = 'home' | 'magvar' | 'cpa' | 'eta' | 'wind' | 'celestial' | 'colreg' | 'draft' | 
            'paint' | 'compass' | 'logbook';
...
function getMenuLabel(key: string, lang: Language): string {
  const labels: Record<string, Record<Language, string>> = {
    ...
    celestial: { id: 'Navigasi Celestial', en: 'Celestial Nav', zh: '天文导航' },
...
function HomePage({ onNavigate }: { onNavigate: (page: Page) => void }) {
  ...
  const menuItems = [
    ...
    { id: 'celestial' as Page, label: getMenuLabel('celestial', lang), icon: Telescope },
  ...
  const getVersionText = () => {
    return `v2.0.0 • WMM2025 • ${lang.toUpperCase()}`;
  };
...
function App() {
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case 'celestial':
        return <CelestialNavigationPanel />;
      case 'colreg':
        return <COLREGPanel />;
      case 'draft':
                        ...
                        <div className="flex items-start gap-4">
                          <div className="flex-shrink-0">
                            <img 
                              src="/profile.jpg" 
                              alt="Developer" 
                              className={`w-20 h-20 rounded-full object-cover border-2 ${isDark ? 'border-[#00eaff]' : 'border-blue-500'} shadow-lg`}
                            />
                          </div>
                          <div className="flex-1">
                            <h3 className={`text-lg font-semibold ${titleColor}`}>ASISTEN OFFICER</h3>
                            <p className={`text-[10px] leading-relaxed ${isDark ? 'text-[#9fe9ff]' : 'text-gray-600'} italic mt-2`}>
                              &ldquo;{lang === 'id' ? 'Di era serba canggih, SOLAS tetap mengharuskan instrumen tradisional ada di kapal; telegraf mengingatkan asal-usul kita, sextant menjaga keterampilan kita. Teknologi berkembang, namun insting pelaut harus tetap tajam. Laut tak pernah berubah dalam tuntutannya—keterampilan, kedisiplinan, dan ketajaman seorang navigator.' :
                                lang === 'zh' ? '在这个先进的时代，SOLAS仍然要求船上配备传统仪器；电报让我们想起起源，六分仪保持我们的技能。技术在发展，但水手的直觉必须保持敏锐。大海对其要求从未改变——技能、纪律和导航员的敏锐。' :
                                "In this advanced era, SOLAS still requires traditional instruments onboard; the telegraph reminds us of our origins, the sextant maintains our skills. Technology evolves, but a sailor's instinct must remain sharp. The sea never changes in its demands—skill, discipline, and the keenness of a navigator."}&rdquo;
                            </p>
                            <p className={`text-[10px] ${isDark ? 'text-[#00eaff]' : 'text-blue-600'} mt-2 text-right`}>⚓ Mr. Wahid</p>
                          </div>
                        </div>
